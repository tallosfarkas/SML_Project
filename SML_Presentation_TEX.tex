...
\begin{itemize}
    ...
    \item \texttt{n.trees = 200} (The number of iterations $M$): This is the optimal stopping point found by CV.  It is directly balanced with the small shrinkage; a slow learner ($\nu=0.01$) requires more iterations ($M=200$) to fit the signal.
...
\scriptsize{
\begin{itemize}
    \item Accuracy: \textbf{0.674}
    \item AUC: \textbf{0.783}
    \item Log-Loss: \textbf{0.5531}
    \item Tuned Threshold: \textbf{0.6962} (via training ROC)
\end{itemize}
}
...
\scriptsize{
\begin{itemize}
    \item \textbf{Specificity}: $33 / (33 + 11) \approx \textbf{75\%}$
    \item \textbf{Sensitivity}: $54 / (54 + 31) \approx \textbf{64\%}$
    \item The additive GBM model shows a good balance, effectively capturing both "Up" and "Down" markets.
\end{itemize}
}
\end{column}
...
\scriptsize{
\begin{itemize}
    \item The model is \textbf{dominated} by a single predictor: \texttt{VIX\_change\_lag} (73.6\% relative influence).
    \item The best-tuned model was additive (\texttt{interaction.depth = 1}), so it only finds main effects.
    \item Other macro (\texttt{CPI\_lag}) and momentum (\texttt{lag3\_return}) features provide minor corrective adjustments to the main volatility signal.
\end{itemize}
}
\end{column}
...
\begin{columns}[T, totalwidth=\textwidth]
 ...
 \begin{column}{0.5\textwidth}
    ...
    \begin{itemize}
        ...
        \item The linear \textbf{Elastic Net} had the highest Test AUC (0.790), narrowly beating the GBM (0.783).
...
\footnotesize % Set base font size for the slide

\textbf{What We Did:}
\begin{itemize} \tiny
    \item We built 3 distinct models (Regularized GLM, Bagging, Boosting) to predict S\&P 500 direction.
    \item We used a \textbf{rolling-window cross-validation} for hyperparameter tuning to respect the time-series nature of the data.
    \item We addressed \textbf{class imbalance} by tuning the probability threshold on the training set.
\end{itemize}
...
\textbf{What We Found:}
\begin{itemize} \tiny
    \item The \textbf{Elastic Net (Tuned)} was the winning model based on our primary metric, Test AUC (0.790). The GBM (0.783) also showed excellent probability ranking.
    \item All models confirmed that \textbf{volatility} (e.g., `VIX\_change\_lag`) and \textbf{momentum} (e.g., `lag1\_return`) are the most dominant predictive signals.
    \item Methodology is critical: standard CV would have failed, and using a 0.5 threshold would have been misleading.
\end{itemize}



% --- [NEW SECTION - FONT SIZE ADJUSTED] ---
\textbf{Implications \& Future Work:}
\begin{itemize} \tiny % Set font size for this itemize
    \item \textbf{Can we predict the market?} Our results show that S\&P 500 monthly direction is \textbf{not} a random walk. There is a persistent, predictable statistical edge (AUC greater than 0.75) that our models can capture.

    \item \textbf{A "Long/Short" Strategy:} This edge could form the basis of a simple allocation strategy:
        \begin{itemize} \tiny % Can make this inner list even smaller if needed
            \item Go \textbf{Long} S\&P 500 when our model's probability > tuned threshold.
            \item Move to \textbf{Cash} (or Short) when probability < tuned threshold.
        \end{itemize}
    \item \textbf{Limitations:} This is not a crystal ball. These models must be continuously re-trained, and we did not account for transaction costs, which are critical in a live trading environment.
\end{itemize}
\end{frame}
...
% --- [MODIFIED COLUMN 1] ---
  \begin{column}{0.5\textwidth}
    ...
    \tiny
    \textbf{Insight:} The probability of an "Up" month is highest (around 0.7) when the VIX has a small \textit{negative} change (slight calming). A large VIX spike (large positive change) dramatically \textit{decreases} the probability of an "Up" month.
  \end{column}
...
\begin{itemize}
    \item \texttt{n.trees ($M$)}: \{100, 200, 300\}
        \begin{itemize} \scriptsize % <-- Set nested list to scriptsize
    ...
    \item \texttt{shrinkage ($\nu$)}: \{0.01, 0.1\}
        ...
        \end{itemize}
\end{itemize}
\textit{Total combinations tested via Time-Series CV: 18}
...
\begin{frame}
\frametitle{Appendix: Strategy Backtest (Strategy and Plot)}
...
\begin{frame}
\frametitle{Appendix: Strategy Backtest (Performance Metrics)}
\footnotesize
