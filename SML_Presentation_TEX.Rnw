...
\begin{itemize}
    ...
    \item Each new, simple tree (a "weak learner") is trained on the residuals (the errors) of the previous trees.
...
\begin{itemize}
    \item \texttt{n.trees = 200} (The number of iterations $M$).
    \item \texttt{interaction.depth = 1} (Tree size $J=1$, making this an additive model with no interactions).
...
\textbf{What We Did:}
% --- [FIXED] Removed blank lines between \item commands ---
\begin{itemize}
    \item We built 3 distinct models (Regularized GLM, Bagging, Boosting) to predict S\&P 500 direction.
    \item We correctly used a \textbf{rolling-window cross-validation} for hyperparameter tuning to respect the time-series nature of the data.
    \item We addressed \textbf{class imbalance} by tuning the probability threshold on the training set (Youden's J).
\end{itemize}
...
\textbf{What We Found:}
\begin{itemize}
    \item The \textbf{Elastic Net (Tuned)} was the winning model based on our primary metric, Test AUC (0.790).
    % --- [FIXED] Escaped the underscores ---
    \item All models found that \textbf{volatility} (e.g., `VIX\_change\_lag`) and \textbf{momentum} (e.g., `lag1\_return`) were highly predictive.
...
%-------------------------------------------------------
\begin{frame}
\frametitle{Appendix: Hyperparameter Grids}
\footnotesize

\textbf{Random Forest Grid:}
\begin{itemize}
    \item \texttt{mtry}
    \item \texttt{min.node.size}
    \item \texttt{max.depth}
    \item \texttt{sample.fraction}
\end{itemize}
\textit{Total combinations tested: 400}
...
\begin{itemize}
    \item \texttt{n.trees}
    \item \texttt{interaction.depth}
    \item \texttt{shrinkage}
\end{itemize}
\textit{Total combinations tested: 18}
